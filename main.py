...
def get_public_ips(endpoints: list) -> set:
    def ip_map(ip: str) -> str:
        match = re.match(
            r"(((?:\d{1,3}\.){3}\d{1,3})|\[((?:[0-9A-Fa-f]{0,4}:){4})(?:[0-9A-Fa-f]{0,4}:){0,3}(?:[0-9A-Fa-f]{0,4})?\])(?::\d+)?",
            ip)
        if not match:
            raise RuntimeError("Invalid IP passed for endpoint: ", ip)

        stripped_ip = f"{match.group(3)}:/64" if match.group(3) else f"{match.group(2)}/32"
        return stripped_ip

    cleaned_ips = set(map(ip_map, endpoints))
    public_ips = set(filter(lambda ip: not re.match(
        r"(^127\.)|(^192\.168\.)|(^10\.)|(^172\.1[6-9]\.)|(^172\.2[0-9]\.)|(^172\.3[0-1]\.)|(^::1$)|(^[fF][cCdD])", ip),
                            cleaned_ips))
    return public_ips
...
def main():
    ...
    ts_expires_at = config_data["ts_expires_at"] and datetime.fromtimestamp(float(config_data["ts_expires_at"]))
    if (
            not ts_expires_at
            or (ts_expires_at - timedelta(minutes=5)) < datetime.now()
    ):
        ...
        token_data = oauth.fetch_token(token_url="https://api.tailscale.com/api/v2/oauth/token", client_id=ts_client_id,
                                       client_secret=ts_client_secret)
        ts_token = token_data["access_token"]
        ts_expires_at = token_data["expires_at"]
        config.save_section("MAIN", {
            "ts_token": ts_token,
            "ts_expires_at": str(ts_expires_at)
        })
        print("Tailscale token refreshed!")

    ts_session = requests.Session()
    ts_session.headers.update({"Authorization": "Bearer " + ts_token})
    devices_req = ts_session.get("https://api.tailscale.com/api/v2/tailnet/-/devices?fields=all")
    devices_req.raise_for_status()

    def device_map(device: dict):
        key = device["hostname"] if not device["hostname"] == "localhost" else device["name"].split(".")[0]
    ...
    device_ips = {k: get_public_ips(v["clientConnectivity"]["endpoints"]) for k, v in approved_devices_data.items()}
    ...
    policy = next((p for p in policies_req.json()["result"] if p["name"].lower() == config_data["policy_name"].lower()),
                  None)
    if not policy:
    ...
    policy_put_req = cf_session.put(
        f"https://api.cloudflare.com/client/v4/accounts/{config_data["cf_account_id"]}/access/policies/{policy["id"]}",
        json=payload)
